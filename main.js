...
window.addEventListener('DOMContentLoaded', () => {
    ...
    list.forEach(h => {
      ...
      Object.entries(aObj)
        ...
        .forEach(([k, v]) => {
          ...
          (effectsByTarget[target] = effectsByTarget[target] || []).push(
            `${sign} ${abbr}`
          );
        });
            ...
            .map(([t, arr]) => {
              const line =
                arr.length === 1
                  ...
                  : `${arr
                      .slice(0, -1)
                      .join(' and ')} and ${arr.at(-1)} to ${t}`;
              return `<p>${line}</p>`;
            })
            .join('')
        : '<p>No effect data.</p>';

      const desc = (aObj.Description || '').replace(
        /\bDESCRIPTION\b\.?$/i,
        ''
      );
  ...
  Promise.all([
    ...
    fetch('archetype.json').then(r => r.json())    // <-- note singular filename
  ])
