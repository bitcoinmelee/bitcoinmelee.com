...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function renderGrid(list) {
    ...
    list.forEach(h => {
      ...
      const statsHtml = 
        `<div style="display:grid;grid-template-columns:45px 45px;gap:1px 1px;justify-items:start;">` +
    ...
    activateFloatingTooltips1();
    activateFloatingTooltips2();
  }
  ...
  function activateFloatingTooltips1() {
    ...
    document.querySelectorAll('.ability-container').forEach(container => {
      const tip = container.querySelector('.tooltip-box-ability');
      if (!tip) return;
